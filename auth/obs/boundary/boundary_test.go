// Package boundary_test guards the module boundary that auth/obs exists to
// create.
//
// lib-auth must not name a github.com/LerianStudio/lib-observability type
// anywhere on its public API. Go types have nominal identity, so a single
// exported field or parameter of a lib-observability type re-pins every
// consumer of lib-auth to whichever lib-observability major lib-auth chose.
// This test walks every non-test .go file in the module with go/ast and fails
// if any exported symbol mentions such a type.
//
...
// parameter position of that library is declared with universal types, so the
// packages here that still import it are calling real functionality (tracing,
// runtime, metrics, zap) and never converting a type across the boundary. No
// package of lib-auth has an excuse. Adding an entry re-opens the coupling
// auth/obs exists to close.
...
func TestPublicAPI_DoesNotMentionLibObservability(t *testing.T) {
	...
	err = filepath.WalkDir(filepath.Join(root, "auth"), func(path string, entry fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		...
		// internal/ packages are not importable by consumers, so they are not
		// part of the public API.
		if strings.Contains(filepath.ToSlash(rel), "/internal/") {
			return nil
		}
	...
	if !assert.Empty(t, violations, "lib-observability types must not appear on the public API of lib-auth") {
